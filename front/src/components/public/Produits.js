            ...
            <div className="produits_parent_container">
                {!refNotfound ?
                    ...
                    <div key={product.id} className='produits_img_englob'>
                        ...
                        <Link to={`/produit_details/${product.id}`}>{<div className='produits_img_container' style={{backgroundImage: `url('http://${process.env.REACT_APP_REMOTE_ADDR}:${process.env.REACT_APP_SERVER_PORT}/uploads/${product.image}')`}}></div>}</Link>
